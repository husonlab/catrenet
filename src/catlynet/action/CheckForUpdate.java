...
public class CheckForUpdate {
    ...
    public static void apply() {
        ...
        try {
            updateDescriptor = UpdateChecker.getUpdateDescriptor("http://software-ab.informatik.uni-tuebingen.de/download/catlynet/updates.xml", applicationDisplayMode);
        } catch (Exception e) {
            Basic.caught(e);
        ...
        if (updateDescriptor.getEntries().length > 0) {
            if (!ProgramProperties.isUseGUI()) {
                ...
                NotificationManager.showInformation("New version available: " + entry.getNewVersion() + "\nPlease download from: http://software-ab.informatik.uni-tuebingen.de/download/catlynet/");
                return;
            }
        } else {
            NotificationManager.showInformation("Installed version is up-to-date");

...
public class CheckForUpdate {
    ...
    public static void apply() {
        ...
        try {
            updateDescriptor = UpdateChecker.getUpdateDescriptor("http://www-ab.informatik.uni-tuebingen.de/data/software/catlynet/download/updates.xml", applicationDisplayMode);
        } catch (Exception e) {
            Basic.caught(e);
        ...
        if (updateDescriptor.getEntries().length > 0) {
            if (!ProgramProperties.isUseGUI()) {
                ...
                NotificationManager.showInformation("New version available: " + entry.getNewVersion() + "\nPlease download from: http://www-ab.informatik.uni-tuebingen.de/data/software/catlynet/download/");
                return;
            }
        } else {
            NotificationManager.showInformation("Installed version is up-to-date");

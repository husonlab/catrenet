/*
 * GreedilyGrowMaxCAF2MaxRAF.java Copyright (C) 2020. Daniel H. Huson
 *
...
public class GreedilyGrowMaxCAF2MaxRAF {
    ...
    public static void apply(MainWindow window, ReactionSystem inputReactionSystem, ChangeListener<Boolean> runningListener) {
        ...
        service.setCallable(() -> {
            ...
            final ReactionSystem maxCAF = (new MaxCAFAlgorithm()).apply(inputReactionSystem, new ProgressSilent()).getCompressedSystem();
            final ReactionSystem maxRAF = (new MaxRAFAlgorithm()).apply(inputReactionSystem, new ProgressSilent()).getCompressedSystem();
            ...
            while (augmentedMaxCAFSize < maxRAF.size()) {
                final Optional<Pair<Reaction, Integer>> best = remainingReactions.parallelStream().map(name -> {
                    ...
                    try {
                        ...
                        final ReactionSystem workingMaxCAF = (new MaxCAFAlgorithm()).apply(workingSystem, new ProgressSilent()).getCompressedSystem();
